...
export default function PublicCategory() {
  ...
  const handleSubmit = (event:any) => {
    fetchData('advertisement/get-all-advertisementsby-search?search='+name+'&city='+cityId+'&ensemble='+idEnsamble+'&category='+idCategory, setApiAdvertisementsData,'advertisement');
  ...
  const handleIlikeIt= (id:any) => {
    ...
    .then((responseData:any) => {
      setIsLoading(false);
      setmsgApi(responseData.msg);
      fetchData('advertisement/get-all-advertisementsby-search?search='+name+'&city='+cityId+'&ensemble='+idEnsamble+'&category='+idCategory+'&page='+actualPage, setApiAdvertisementsData,'advertisement');
      if(responseData.success == true) {
      } else {
        setShowModalError(true);
      }
  ...
  const loadPage = (page:number) => {
    setActualPage(page);
    fetchData('advertisement/get-all-advertisementsby-search?search='+name+'&city='+cityId+'&ensemble='+idEnsamble+'&category='+idCategory+'&page='+page, setApiAdvertisementsData,'advertisement');
    setCurrentPage(page);
  };
  ...
  useEffect(() => {
    ...
    const idEnsamble = searchParams.get('sets');
    const idCategory = searchParams.get('category');

    setName(name || ''); // Si name es nulo, establecer una cadena vacía
    setCityId(cityId || '');
    setIdEnsamble(idEnsamble || '');
    setIdCategory(idCategory || '');

    loadPage(1);
    fetchData('city/list/everything', setApiCityData,'city');
    fetchData('category/list', setApiCategoryData,'category');
    fetchData('advertisement/get-all-advertisementsby-search?search='+name+'&city='+cityId+'&ensemble='+idEnsamble+'&category='+idCategory+'&page='+1, setApiAdvertisementsData,'advertisement');
  },[]);
      ...
      <div className="bg-white py-4 py-sm-5">
        <div className="container py-md-3">
          <div className="row gx-lg-5">
            <ListAds
              setApiAdvertisementsData={setApiAdvertisementsData} apiAdvertisementsData={apiAdvertisementsData}
